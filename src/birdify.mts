...
try {
  ...
  if (command === 'doctor') {
    if (args.length) throw new Error('Usage: birdify doctor');
    ...
    while (args.length) {
      ...
      if (!flag || flag !== '--project' || !value || value.startsWith('--') || seen.has(flag)) throw new Error(usage);
      seen.add(flag);
      root = path.resolve(value);
    }
    if (mode && !['auto', 'on-demand', 'off'].includes(mode)) throw new Error(usage);
