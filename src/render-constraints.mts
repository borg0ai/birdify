...
import type { ConstraintCatalog, ConstraintGraph, ConstraintGraphNode, ConstraintSource, ReviewedConstraintCatalog, SourceHistory } from './constraint-types.mjs';
...
export function buildConstraintGraph(catalog: ConstraintCatalog, { view = 'rules', sourceHref }: { view?: 'rules' | 'sources'; sourceHref?: string } = {}): ConstraintGraph {
  ...
  const nodes: ConstraintGraphNode[] = [], documents: Record<string, { body: string }> = {}, ids = new Set<string>();
  const revision = catalog.project.revision;
  const add = (id: string, parent: string | null, title: string, desc: string, body: string, history?: SourceHistory): void => {
    if (ids.has(id) || (parent && !ids.has(parent))) throw new Error(`Duplicate id or missing parent: ${id}`);
    ids.add(id);
  ...
  for (const [id, title] of [['instruction', '目录指令'], ['skill', '任务技能'], ['reference', '引用规范']] as const) {
    add(id, 'project', title, '按实际来源展开；不自动判定生效。', `# ${title}\n\n${introduction}`);
  }
  const paths = new Set(catalog.sources.map(source => source.path));
  if (paths.size !== catalog.sources.length) throw new Error('Duplicate source path');
  for (const source of catalog.sources) {
    if (!['instruction', 'skill', 'reference'].includes(source.kind)) throw new Error('Unknown source kind');
    const scope = source.scope ?? '按任务和引用关系判断 / Conditional';
    let parent: string = source.kind;
    ...
    for (let index = 0; index < folders.length; index++) {
      ...
      if (!ids.has(id)) add(id, parent, folders[index]!, folder, `# ${folder}\n\n${introduction}`);
      parent = id;
    }
    add(source.id, parent, path.posix.basename(source.path), `范围：${scope} · 适用性：${source.applicability}`,
      `# ${source.path}\n\n范围：${scope}\n\n来源于：${source.discoveredFrom || '本地约束入口'}\n\n${introduction}\n\n## 原文 / Source\n\n${source.text}`, source.history);
    const ancestors: Array<{ id: string; level: number }> = [];
    for (const section of source.sections) {
      while (ancestors.length && ancestors.at(-1)!.level >= section.level) ancestors.pop();
      add(section.id, ancestors.at(-1)?.id || source.id, section.title === catalog.project.name ? `文档：${section.title}` : section.title,
        `${source.path}:${section.line}-${section.endLine} · 待审查 / Unreviewed`,
        `来源：\`${source.path}:${section.line}-${section.endLine}\`\n\n${introduction}\n\n${section.text}`);
  ...
  for (const rule of catalog.rules || []) {
    ...
    if (!source || !rule.id || !rule.name || !rule.explanation || !rule.condition || !rule.verification
      ...
      || !['applicable', 'conditional', 'superseded', 'conflict', 'uncertain'].includes(rule.applicability)) throw new Error('Invalid reviewed rule');
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  ...
  if (!input || !output) throw new Error('Usage: node scripts/render-constraints.mjs catalog.json constraints.html');
