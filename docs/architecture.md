...
1. **`cli` (Birdify CLI & Doctor)**
   ...
   - **Key files**: `apps/cli/src/birdify.ts`
...
2. **`validator` (Schema & Graph Validator)**
   ...
   - **Key files**: `apps/cli/src/commands/validate.ts`, `packages/core/src/validate.ts`
...
3. **`contracts` (Data Contracts & Schemas)**
   ...
   - **Key files**: `birdify/schemas/architecture.schema.json`, `birdify/schemas/activity.schema.json`, `src/contracts/models.ts`
...
4. **`renderer` (HTML Snapshot Renderer)**
   ...
   - **Key files**: `apps/cli/src/commands/render.ts`, `packages/core/src/render.ts`
...
5. **`constraint-engine` (Constraint Extraction & Rule Graph)**
   ...
   - **Key files**: `apps/cli/src/commands/constraints.ts`, `packages/core/src/discover-constraints.ts`, `packages/core/src/compile-constraint-rules.ts`, `packages/core/src/render-constraints.ts`
...
6. **`viewer-ui` (Interactive Canvas Viewer Runtime)**
   ...
   - **Key files**: `templates/viewer/main.ts`, `toolings/birdify-dev/src/commands/build-viewer.ts`, `birdify/assets/viewer.js`, `birdify/assets/viewer.css`, `birdify/assets/theme.js`, `birdify/assets/constraint-canvas.js`
...
7. **`site-app` (Landing Site Application)**
   ...
   - **Key files**: `apps/site/index.html`, `apps/site/src/App.vue`, `apps/site/vite.config.ts`
...
```bash
pnpm exec birdify validate .birdify/architecture.json --authoring --bilingual
```
...
pnpm exec birdify render-constraints .birdify/constraints.sources.json .birdify/constraints.html --sources
```
