...
try {
    for (const module of ['routing', 'i18n', 'main', 'constraint-canvas', 'theme']) {
        const format = module === 'routing' || module === 'i18n' ? 'esm' : 'iife';
        const target = module === 'main' ? 'assets/viewer.js' : module === 'constraint-canvas' ? 'assets/constraint-canvas.js' : module === 'theme' ? 'assets/theme.js' : `scripts/viewer/${module}.mjs`;
        const source = `src/viewer/${module}.mts`;
        ...
        const files = [path.join(packageRoot, target)];
    ...
    for (const icon of icons) {
        ...
        if (!fs.existsSync(source))
            throw new Error(`Missing source icon: ${icon}.svg`);
        if (check) {
