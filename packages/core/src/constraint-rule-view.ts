import { validateRuleHistory } from './constraint-rule-history.js';
import type { ConstraintGraph, ConstraintGraphNode, ConstraintRole, ConstraintRule, ReviewedConstraintCatalog, RuleApplicability } from './constraint-types.js';
...
export function buildRuleGraph(catalog: ReviewedConstraintCatalog, sourceHref?: string): ConstraintGraph {
  ...
  const counts: Record<string, number> = Object.fromEntries(categories.map(category => [category.id, catalog.rules.filter(rule => rule.category === category.id).length]));
  ...
  const groupRole = (rules: ConstraintRule[]): ConstraintRole => {
    const roles = new Set(rules.map(rule => rule.targetRole || 'generic'));
    return roles.size === 1 ? [...roles][0]! : 'generic';
  };
  const add = (id: string, parent: string | null, title: string, desc: string, body: string, role: ConstraintRole, label: string): void => {
  ...
  for (const [index, rule] of catalog.rules.entries()) {
    ...
    const states: Record<RuleApplicability, string> = { applicable: '适用', conditional: '按条件适用', superseded: '已被替代', conflict: '存在冲突', uncertain: '适用性待确认' };
    ...
    if (history) Object.assign(node, { version: history.version, lastEdited: history.lastEdited });
  ...
  const directoryNodes: ConstraintGraphNode[] = [{ ...nodes[0]! }];
  const directoryIds = new Map<string, string>([['.', 'directory-root']]);
  const directoryNode = (directory: string): string => {
    if (directoryIds.has(directory) && directory !== '.') return directoryIds.get(directory)!;
    const id = directoryIds.get(directory) || `directory-${directoryIds.size}`;
    if (directoryNodes.some(node => node.id === id)) return id;
    ...
    const describe = (sources: ReviewedConstraintCatalog['sources']): string => sources.map(source => {
