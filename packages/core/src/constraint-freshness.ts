...
import type { ConstraintFreshness, FreshnessStatus } from './constraint-types.js';
...
export function inspectConstraintFreshness(map: Architecture, repository: string): ConstraintFreshness {
  ...
  const git = (...args: string[]): string => {
    ...
    if (result.error || result.status !== 0) throw new Error('Git inspection unavailable.');
  ...
  for (const rule of map.constraints || []) {
    const result: ConstraintFreshness['rules'][string] = {
      status: 'unverified', ...(rule.baselineCommit ? { baselineCommit: rule.baselineCommit } : {}), files: []
    };
    report.rules[rule.id] = result;
    if (!rule.baselineCommit) { result.reason = 'no-baseline'; continue; }
    try { git('cat-file', '-e', `${rule.baselineCommit}^{commit}`); }
    catch { result.reason = 'unavailable-baseline'; continue; }
    for (const [role, files] of [['source', rule.evidence], ['code', rule.code || []]] as const) {
      for (const file of files) {
        const key = `${rule.baselineCommit}:${file.path}`;
        if (!cache.has(key)) {
          let status: FreshnessStatus = 'unverified';
