...
export function discoverConstraints(repository, { title, maxSources = 1000 } = {}) {
    if (!Number.isInteger(maxSources) || maxSources < 1)
        throw new Error('maxSources must be a positive integer');
    const git = (...args) => execFileSync('git', ['--no-optional-locks', '-C', repository, ...args], {
    ...
    const modes = new Map(tree.map(entry => [entry.slice(entry.indexOf('\t') + 1), entry.slice(0, 6)]));
    ...
    const exclusion = (file) => {
        if (/(^|\/)(fixtures?|__fixtures__|node_modules)(\/|$)/i.test(file) || /(^|\/)snapshots\/.*\/workspace\//.test(file))
            return 'fixture-or-dependency';
        if (/(^|\/)(archived?|archive)(\/|$)/i.test(file))
            return 'historical';
        if (/\.zh\.md$/.test(file) && tracked.has(file.replace(/\.zh\.md$/, '.md')))
            return 'translation-counterpart';
        return null;
    };
    const queue = [];
    const seen = new Set();
    const references = [];
    const unresolved = [];
    const enqueue = (file, from = null) => {
        const reason = exclusion(file);
        if (reason) {
            excluded.set(file, { path: file, reason });
            return;
        }
        if (!seen.has(file)) {
            seen.add(file);
    ...
    for (let index = 0; index < queue.length && sources.length < maxSources; index++) {
        ...
        if (modes.get(file) === '120000') {
            ...
            if (!text.startsWith('/') && !target.startsWith('../') && tracked.has(target))
                enqueue(target, file);
            else
                unresolved.push({ from: file, target: text, reason: 'outside-or-missing-alias' });
            continue;
        }
        ...
        for (let n = 0; n < lines.length; n++) {
            ...
            if (heading)
                headings.push({ line: n + 1, level: heading[1].length, title: heading[2] });
            prose.push(line);
        }
        ...
        for (const target of new Set(targets.filter((value) => value !== undefined))) {
            ...
            try {
                ...
                unresolved.push({ from: file, target, reason: 'invalid-url' });
                continue;
            }
            if (!/\.(md|mdc)$/.test(decoded))
                continue;
            let resolved = path.posix.normalize(path.posix.join(path.posix.dirname(file), decoded));
            if (!tracked.has(resolved) && tracked.has(decoded))
                resolved = decoded;
            if (decoded.startsWith('/') || resolved.startsWith('../') || !tracked.has(resolved)) {
                unresolved.push({ from: file, target, reason: 'outside-or-missing' });
                continue;
            }
        ...
        if (!sections.length)
            sections.push({ id: `${id}-s1`, title: path.posix.basename(file), level: 1, line: 1, endLine: lines.length, text });
    ...
    for (const entry of history.split('\x1e').filter(Boolean)) {
        ...
        for (const file of new Set(changed)) {
            ...
            if (!source)
                continue;
            if (!shallow)
                source.history.version = (source.history.version ?? 0) + 1;
            if (!source.history.commit)
                Object.assign(source.history, { commit: commit, lastEdited: date });
        }
    }
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
    ...
    if (!repository || !output)
        throw new Error('Usage: node scripts/discover-constraints.mjs repository catalog.json [project-name]');
