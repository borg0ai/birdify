...
这是用于演示 Birdify 的概念架构，不是 Birdify 实现或某个具体编码 Agent 的地图。示例中所有 `src/` 归属路径仅供说明。supported 状态表示作者编写的示例设计，不表示检查过生产代码。没有连接实时执行。
...
地图的检查路径指向这份本地来源摘要及其译文。上游检查仅覆盖上述两个文件，未检查包级指令及实现遵守情况。模块关联与建议验证方式是 Birdify 编写的投影，不是额外的上游要求。
